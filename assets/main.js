(() => {
  ...
  const projects = {
    ...
    credit: { category: '02 / AGENTIC AI', heading: 'Decisions with<br>a paper trail.', description: 'Five agents take a loan from ingestion to audit. Policy retrieval, SHAP explanations, and human checkpoints make each decision traceable.', metric: '0.7459', label: 'FORM-INPUT TEST ROC-AUC', caption: 'INGESTION → RISK → POLICY → DECISION → AUDIT', demo: 'demos/credagent.html', alt: 'Illustrative five-agent credit underwriting pipeline' },
    ...
    fraud: { category: '03 / MACHINE LEARNING', heading: 'Find the signal.<br>Catch the anomaly.', description: 'XGBoost and IsolationForest work together to identify known and novel fraud, with a scoring API, stream simulator, and operations dashboard.', metric: '0.8755', label: 'PR-AUC AT 0.17% FRAUD RATE', caption: 'TRANSACTIONS → SCORING → ANOMALY SIGNALS', demo: 'https://fraud-pulse.vercel.app', alt: 'Illustrative transaction stream with highlighted anomaly signals' }
  };
  let selected = 'vision';
